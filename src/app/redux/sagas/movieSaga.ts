...
function* fetchMovies() {
  try {
    const response: AxiosResponse = yield call(axios.get, `http://www.omdbapi.com/?apikey=${API_KEY}&s=avengers`);
...
function* fetchPopularMovies(action: { type: string; payload: { page: number } }) {
  try {
    ...
    const response: AxiosResponse = yield call(axios.get, `http://www.omdbapi.com/?apikey=${API_KEY}&s=movie&y=2024&page=${page}`);
...
function* fetchMovieDetail(action: FetchMovieDetailAction) {
  try {
    const response:AxiosResponse = yield call(axios.get, `http://www.omdbapi.com/?apikey=${API_KEY}&i=${action.payload}`);
...
function* searchMovies(action: SearchMoviesAction) {
  try {
    const response:AxiosResponse = yield call(axios.get, `http://www.omdbapi.com/?apikey=${API_KEY}&s=${action.payload}`);
